...
def find_contours(dilationimage):
	...
	for cnt in contours:
		...
		coin = input_image[int(y*width/FRAME_WIDTH) : int((y + h)*width/FRAME_WIDTH), int(x*width/FRAME_WIDTH) : int((x + w)*width/FRAME_WIDTH)]
...
def classify(coinimage):
	...
	my_image_resized = cv2.resize(coin_rgb, (img_size, img_size))
...
# Load the model
if os.path.isfile('EuroCNN100.h5'):
	model = load_model('EuroCNN100.h5')
	print ("Model loaded")
else:
